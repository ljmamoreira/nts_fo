...
leis da óptica geométrica: a interferência, a difração e a polarização da luz.
...
que, veremos, permite também deduzir as próprias leis da óptica geométrica. É,
...
A Figura~\ref{fig:20-030} mostra um impulso ondulatório que se propaga ao longo
...
A \emph{frequência} de uma onda harmónica é o número de máximos da onda que
...
A representação complexa das ondas harmónicas é usada porque simplifica imenso
...
\begin{figure}[htb]
  {\centering
    \begin{tikzpicture}
      ...
      \draw [thin, ->] (o) -- (10:3.5) node [above] {$s$};
    \end{tikzpicture}\par
  }
...
Uma vez que as fretes de onda de uma onda esférica têm a forma de esféricas
...
à posição da fonte. No caso de ondas harmónicas, esta expressão toma a forma
mais concreta
\begin{equation}
\psi(\vec r,\,t)=\frac{A}{r}e^{\iu(kr-\omega t+\phi_0)}.
\end{equation}
A amplitude é dividida pela distância à fonte ($r$) para dar conta do facto da
intensidade das ondas eféricas diminuir com o quadrado dessa distância. Uma vez
que a intensidade é proprcional ao quadrado da função de onda, esta modificação
da amplitude é a adequada para se descrever esse efeito.
