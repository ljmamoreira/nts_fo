...
cartesianas, $f(\vec r)=f(x,y,z)$. Consideremos a sua variação num deslocamento
infinitesimal $\smash{\delta\vec r}$ ao longo da direção de um versor $\hat u$,
partindo de um ponto $P=(x_0,y_0,z_0)$. De acordo com a definição de derivada
direcional da eq.~\eqref{eq:ddir}, a variação de $f$ neste deslocamento é
\begin{equation}\label{eq:ddgrad1}
  \delta f = f'_{\hat u}(P)\,\delta s,
\end{equation}
onde $\smash{\delta s=\|\delta \vec r\|}$ é o comprimento do deslocamento
  $\smash{\delta \vec r}$.
Por outro lado, de acordo com a eq.~\eqref{eq:df}, essa mesma variação é dada
por
\begin{equation*}
  \delta f= \pd{f}{x}\delta x+ \pd{f}{y}\delta y+ \pd{f}{z}\delta z;
\end{equation*}
mas $\delta x$, $\delta y$ e $\delta z$ são as componentes cartesianas do vetor
deslocamento, que considerámos na direção do versor $\hat u$, ou seja
\begin{equation*}
  \delta x = (\delta \vec r)_x=(\delta s\hat u)_x=\delta s u_x,
\end{equation*}
onde $u_x$ representa a componente cartesiana do versor $\hat u$ segundo a
direção $x$. Procedendo da mesma maneira para as restantes componentes, obtemos
\begin{equation*}
  \delta f=\left( \pd{f}{x}u_x+ \pd{f}{y}u_y+ \pd{f}{z}u_z \right)\delta s.
\end{equation*}
Comparando esta igualdade com a da eq.~\eqref{eq:ddgrad1} uma expressão para o
cálculo da derivada direcional de uma função expressa em coordenadas
cartesianas:
\begin{equation}
  f'_{\hat u}= u_x\pd{f}{x}+ u_y\pd{f}{y}+ u_z\pd{f}{z}.
\end{equation}
