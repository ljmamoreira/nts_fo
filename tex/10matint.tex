...
mais conveniente. Qualquer problema pode ser analizado com qualquer sistema de
...
\item\textbf{coordenadas polares esféricas $(r,\,\theta,\,\phi)$}\\
    Outro sistema muito usual é o das coordenadas polares esféricas. A posição
    de um ponto é indicada pela sua distância à origem, pelo ângulo (chamado
    \emph{ângulo polar}) que o seu vetor posição faz com o eixo dos $z$ (que toma
    o nome de \emph{eixo polar} no sistema das coordenadas esféricas) e pelo
    ângulo (\emph{ângulo azimutal})  que a projeção do seu vetor posição no
    plano $xy$ (plano \emph{equatorial}) faz com o eixo dos $x$.
\item\textbf{coordenadas polares cilíndricas $(\rho,\,z,\,\phi)$}\\
    ...
    das coordenadas cartesianas e das coordenadas polares esféricas. Neste
    sistema de coordenadas usa-se a distância do ponto ao eixo dos $z$ (atenção:
    é a distância ao eixo, não a distância à origem), a própria coordenada
    cartesiana $z$ e o ângulo azimutal para especificar a sua posição.
\end{itemize}
\begin{figure}[htb]
{\centering
...
\begin{tikzpicture}[scale=3,tdplot_main_coords]
    ...
    \begin{scope}
        ...
        \draw[thick,tdplot_rotated_coords,->] (0,0,0)
            -- (.15,0,0) node[right]{$\hat u_\theta$};
        \draw [thick,tdplot_rotated_coords,->] (0,0,0) -- (0,0.14,0)
            node[right]{$\hat u_\phi$};
...
\subsection{Derivadas direcionais}
...
como vimos), mas temos que ter em conta que a variável é agora a posição e,
partindo de um dado ponto, podem considerar-se acréscimos (pequenas variações)
dessa variável em diferentes direções: em vez de $x+\delta x$, devemos agora
considerar $\smash{\vec r+\delta \vec r}$ e ter presente que este acréscimo
$\smash{\delta \vec r}$ é caracterizado por uma grandeza, mas também por uma
orientação. Por isso, uma ``adaptação à letra'' da eq.~\eqref{eq:dfdx},
